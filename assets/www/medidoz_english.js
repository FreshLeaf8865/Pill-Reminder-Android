...
//        }
    ...
    //Google Analytics
    window.plugins.analytics.start("UA-37532539-2", 
...
function createTable(tx) {
    ...
    tx.executeSql(sqlStr, [], onSqlSuccess_createTable, onSqlError_createTable);
}


function onSqlSuccess_createTable(tx, res) {
    if(res) 
    {
        console.log("onSqlSuccess_createTable, res: " +  res);
    }
}


function onSqlError_createTable(tx, err) {
    var msgText;
    if(err) 
    {
        msgText = "onSqlError_createTable: " + err.message + " (" + err.code + ")";
    } 
    else 
    {
        msgText = "onSqlError_createTable: Unknown error";
    }
...
//            $("#listReminders").show(); 
    ...
    {
        console.log("onQuerySuccess_listReminders: No records match selection criteria.");
    }
}
...
function insertOrUpdateRecord(tx) {
    ...
    if (updateMode) {
        ...
        sqlStr = "UPDATE MEDICINE SET medicineName = '" + tmpName.toUpperCase() + "', medicineType = '" + tmpType 
            ...
            + "' WHERE id = " + id;
        console.log("sql update statement: " + sqlStr);
        //Instead of checking the count of daily takes, simply delete all possible alarm id's.
        plugins.localNotification.cancel((id * 100) + 1); 
        plugins.localNotification.cancel((id * 100) + 2);
        plugins.localNotification.cancel((id * 100) + 3);
        plugins.localNotification.cancel((id * 100) + 4);
        plugins.localNotification.cancel((id * 100) + 5);          
        tx.executeSql(sqlStr, [], onSqlSuccess_insertOrUpdateRecord, onSqlError_insertOrUpdateRecord);
    }
    else {
        ...
        tx.executeSql(sqlStr, [tmpDateSaved, tmpName.toUpperCase(), tmpType, tmpDosage, tmpDailyTakeCount, tmpDailyTakeHours, tmpPeriod], 
            onSqlSuccess_insertOrUpdateRecord, onSqlError_insertOrUpdateRecord);
    }
        
}


function onSqlError_insertOrUpdateRecord(tx, err) {
    var msgText;
    if(err) 
    {
        msgText = "onSqlError_insertOrUpdateRecord: " + err.message + " (" + err.code + ")";
    } 
    else 
    {
        msgText = "onSqlError_insertOrUpdateRecord: Unknown error";
    }
    console.error(msgText);
}
...
function onSqlSuccess_insertOrUpdateRecord(tx, res) {
    ...
    if (updateMode) {
        ...
        window.plugins.analytics.trackEvent("User", "Add medicine", tmpName_, 1, function(){console.log("Track: add medicine: success");}, function(){console.log("Track: add medicine: failure");});
    }
    for (var i = 0; i < tmpCount; i++)
    {
        if (tmpCount == 1)
        {
            ...
            if (typeof plugins !== "undefined") {
                plugins.localNotification.add({
                    date : tmpDate0,
                    message : "Medidoz - Pill Reminder\r\n" + tmpHours.substr(0, 2) + ":" 
                        + tmpHours.substr(3, 2) + " " + tmpName_, 
                    ticker : "Medidoz - Pill Reminder",
                    repeatPeriod : tmpPeriod,
                    id : alarmId
                });
            }
        }
        if (tmpCount == 2)
        {
            ...
            if (typeof plugins !== "undefined") {
                plugins.localNotification.add({
                    date : tmpDate0,
                    message : "Medidoz - Pill Reminder\r\n" + tmpHours.substr(0, 2) + ":" 
                        + tmpHours.substr(3, 2) + " " + tmpName_ + " - 1st dose", 
                    ticker : "Medidoz - Pill Reminder",
                    repeatPeriod : tmpPeriod,
                    id : alarmId
                });
            ...
            if (typeof plugins !== "undefined") {
                plugins.localNotification.add({
                    date : tmpDate1,
                    message : "Medidoz - Pill Reminder\r\n" + tmpHours.substr(6, 2) + ":" 
                        + tmpHours.substr(9, 2) + " " + tmpName_ + " - 2nd dose", 
                    ticker : "Medidoz - Pill Reminder",
                    repeatPeriod : tmpPeriod,
                    id : alarmId
                });
        ...
        if (tmpCount == 3)
        {
            ...
            if (typeof plugins !== "undefined") {
                plugins.localNotification.add({
                    date : tmpDate2,
                    message : "Medidoz - Pill Reminder\r\n" + tmpHours.substr(12, 2) + ":" 
                        + tmpHours.substr(15, 2) + " " + tmpName_ + " - 3rd dose", 
                    ticker : "Medidoz - Pill Reminder",
                    repeatPeriod : tmpPeriod,
                    id : alarmId
                });
        ...
        if (tmpCount == 4)
        {
            ...
            if (typeof plugins !== "undefined") {
                plugins.localNotification.add({
                    date : tmpDate3,
                    message : "Medidoz - Pill Reminder\r\n" + tmpHours.substr(18, 2) + ":" 
                        + tmpHours.substr(21, 2) + " " + tmpName_ + " - 4th dose", 
                    ticker : "Medidoz - Pill Reminder",
                    repeatPeriod : tmpPeriod,
                    id : alarmId
                });
        ...
        if (tmpCount == 5)
        {
            ...
            if (typeof plugins !== "undefined") {
                plugins.localNotification.add({
                    date : tmpDate4,
                    message : "Medidoz - Pill Reminder\r\n" + tmpHours.substr(24, 2) + ":" 
                        + tmpHours.substr(27, 2) + " " + tmpName_ + " - 5th dose",
                    ticker : "Medidoz - Pill Reminder",
                    repeatPeriod : tmpPeriod,
                    id : alarmId
                });
...
function deleteMedicine(id) {
    ...
    if (answer)
    {
        ...
        theDB.transaction(function(tx) { 
            tx.executeSql(sqlStr, [], onDeleteSuccess, onSqlError_deleteMedicine);
        }, onTxError, onTxSuccess);
    }
}
...
function onDeleteSuccess(tx, results) {        
    //Instead of checking the count of daily takes, simply delete all possible alarm id's.
    plugins.localNotification.cancel((tmpId * 100) + 1); 
    plugins.localNotification.cancel((tmpId * 100) + 2);
    plugins.localNotification.cancel((tmpId * 100) + 3);
    plugins.localNotification.cancel((tmpId * 100) + 4);
    plugins.localNotification.cancel((tmpId * 100) + 5);    
    listMedicines();
}


function onSqlError_deleteMedicine(tx, err) {
    var msgText;
    if(err) 
    {
        msgText = "onSqlError_deleteMedicine: " + err.message + " (" + err.code + ")";
    } 
    else 
    {
        msgText = "onSqlError_deleteMedicine: Unknown error";
    }
    console.error(msgText);
}
...
function onQuerySuccess_listMedicines(tx, results) {
    ...
    {
        console.log("No records match selection criteria.");
    }
}
...
function soundSettingChanged()
{
    ...
    preferences.set("soundOption", soundOption, function() {
            console.log("soundOption successfully saved.");
        }, function(error) {
            console.log("Error! " + JSON.stringify(error));
    });
}
...
function vibrationSettingChanged()
{
    ...
    preferences.set("vibrationOption", vibrationOption, function(value) {
            console.log("vibrationOption successfully saved.");
        }, function(error) {
            console.log("Error! " + JSON.stringify(error));
    });
}
...
//            console.log("lightsOption successfully saved.");
//        }, function(error) {
//            console.log("Error! " + JSON.stringify(error));
//    });
//}
...
function onQuerySuccess_getMedicineData(tx, results) {
    ...
    else
    {
        ...
        console.log("onQuerySuccess_getMedicineData: No records match selection criteria.")
    }
}
