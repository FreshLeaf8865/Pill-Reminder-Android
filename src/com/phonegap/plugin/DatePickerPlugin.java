...
public class DatePickerPlugin extends Plugin {
	...
	private final class TimeSetListener implements OnTimeSetListener {
		...
		public void onTimeSet(final TimePicker view, final int hourOfDay, final int minute) {
			...
			Log.d("-*-*-*-*- date.toLocaleString(): ", date.toLocaleString());
			Log.d("-*-*-*-*- date.toString(): ", date.toLocaleString());
			
			String tmpHours, tmpMins;
			if (date.getHours() < 10)
			{
				tmpHours = "0" + String.valueOf(date.getHours());
			}
			else
			{
				tmpHours = String.valueOf(date.getHours());
			}
			if (date.getMinutes() < 10)
			{
				tmpMins = "0" + String.valueOf(date.getMinutes());
			}
			else
			{
				tmpMins = String.valueOf(date.getMinutes());
			}
			String returnVal = tmpHours + ":" + tmpMins;
			
			datePickerPlugin.success(new PluginResult(PluginResult.Status.OK, returnVal), callBackId);
