...
public class LocalNotification extends Plugin {
    ...
    public PluginResult execute(String action, JSONArray optionsArr, String callBackId) {
		...
		if (action.equalsIgnoreCase("add")) {
		    final int period = alarmOptions.getRepeatPeriod();
		    ...
		    return this.add(period, title, subTitle, ticker, alarmId, alarmOptions.getCal());
     ...
     * 
     * @param repeatPeriod
     *            The period to be set as the alarm interval.
     * @param alarmTitle
     ...
     */
    public PluginResult add(int repeatPeriod, String alarmTitle, String alarmSubTitle, String alarmTicker,
	    String alarmId, Calendar cal) {
		final long triggerTime = cal.getTimeInMillis();
		//final String recurring = repeatDaily ? "daily" : "onetime";
	
		Log.i(PLUGIN_NAME, "Adding notification with period: " + repeatPeriod + ", title-subtitle: "
			+ alarmTitle + alarmSubTitle + "' with id: " + alarmId + " at timestamp: " + triggerTime );
	
		boolean result = alarm.addAlarm(repeatPeriod, alarmTitle, alarmSubTitle, alarmTicker, alarmId, cal);
		if (result) {
